...
impl Processor {
  ...
  fn create_ticket(
    ...
    ticket:Ticket) -> ProgramResult {
      ...
      let user_address_check: Pubkey = Pubkey::new_from_array(user_account_data.user_address);
     ...
     if n!=3{panic!()}  
      ...
      let new_ticket_data: Ticket = Ticket{
        user_address:user_account_data.user_address, //48
        is_init:1,
  ...
  fn mark_ticket(
    ...
    program_id:&Pubkey) -> ProgramResult {
      ...
      let user_address_check: Pubkey = Pubkey::new_from_array(ticket_data.user_address);
      if &user_address_check != user_address.key{panic!()}
      ...
      let w: WinnersCheck = WinnersCheck{
        user_address:ticket_data.user_address, //48
  ...
  fn delete_ticket(
    accounts: &[AccountInfo]) -> ProgramResult {
      ...
      let user_address_check2: Pubkey = Pubkey::new_from_array(user_account_data.user_address);
  ...
  fn create_account(
    ...
    useraccount:UserAccount) -> ProgramResult {
      ...
      let user_account_data: UserAccount = UserAccount{
        user_address:user_address.key.to_bytes(), //48
  ...
  fn claim_prize(
    accounts: &[AccountInfo]) -> ProgramResult {
      ...
      let user_address_check: Pubkey = Pubkey::new_from_array(winners_check.user_address);
      if &user_address_check != user_address.key{panic!()}
